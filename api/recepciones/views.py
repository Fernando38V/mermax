...
class RecepcionesPendientesListView(APIView):
    ...
    def get(self, request):
        ...
        for m in mermas:
            data.append({
                'folio': m.folio,  # el folio ES el identificador, no hay 'id' numérico
                'fecha': m.fecha,
                'cantidad': m.cantidad,
                'unidad': m.unidad,
...
class ConfirmarRecepcionAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Guarda de estado: evita doble confirmación / doble solicitud de inspección
        if merma.edo_flujo_merma_id != EDO_FLUJO_REGISTRADA:
            return Response(
                {'error': f'La merma {folio} ya no está en estado REGISTRADA '
                          f'(estado actual: {merma.edo_flujo_merma_id}). No se puede confirmar de nuevo.'},
...
class DiscrepanciaCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if registro_merma_folio:
            ...
            # Guarda de estado: no se registra una discrepancia sobre una merma
            # que ya salió de REGISTRADA (ya fue confirmada o ya tiene otra discrepancia)
            if merma.edo_flujo_merma_id not in (EDO_FLUJO_REGISTRADA, EDO_FLUJO_DISCREPANCIA):
                return Response(
                    {'error': f'La merma {registro_merma_folio} está en estado '
                              f'{merma.edo_flujo_merma_id}; no se puede registrar una discrepancia.'},
                    status=status.HTTP_409_CONFLICT,
                )
        ...
        ahora = datetime.now()
        folio_discrepancia = f"DSC-{ahora.strftime('%y%m%d%H%M%S')}"
        ...
        if merma:
            try:
                ...
                return Response(
                    {'error': f'Catálogo edo_flujo_merma "{EDO_FLUJO_DISCREPANCIA}" no configurado.'},
...
class ResolverDiscrepanciaAPIView(APIView):
    ...
    def post(self, request, folio_discrepancia):
        ...
        # Guarda de estado: no resolver dos veces la misma discrepancia
        if discrepancia.edo_discrepancia_id == EDO_DISCREPANCIA_RESUELTA:
            return Response(
                {'error': f'La discrepancia {folio_discrepancia} ya está resuelta.'},
                status=status.HTTP_409_CONFLICT,
            )
        ...
        try:
            ...
            return Response(
                {'error': f'Catálogo edo_discrepancia "{EDO_DISCREPANCIA_RESUELTA}" no configurado.'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
